...
int option_find_int(list_darknet *l, char *key, int def);
int option_find_int_quiet(list_darknet *l, char *key, int def);
...
//det_box *yolo_detect(char *cfgfile,char *weightfile,image im,float thresh,int* num_detect);


network *get_load_network(char *cfgfile,char *weightfile);  //预加载网络
det_box *yolo_detect(image im,network* net,float thresh,int* num_detect);   //开始检测，必须在加载网络之后
