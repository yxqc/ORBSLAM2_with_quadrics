...
FrameDrawer::FrameDrawer(Map *pMap) : mpMap(pMap)
{
    ...
    for (int i = 0; i < 80; i++)
    {
        box_color.push_back(cv::Scalar(rand() % 127 + 128, rand() % 127 + 128, rand() % 127 + 128));
    }
}
    ...
    else if (state == Tracking::OK) //TRACKING
    {
        ...
        for (int i = 0; i < n; i++)
        {
            if (vbVO[i] || vbMap[i])
            {
                ...
                if (point_Object_AssoID.size() > 0)
                    if (point_Object_AssoID[i] > -1)
                        //cv::circle(im, vCurrentKeys[i].pt, 4, box_color[point_Object_AssoID[i] % box_color.size()], -1);
                        cv::circle(im, vCurrentKeys[i].pt, 4, box_color[0], -1);
            }
        }
...
void FrameDrawer::Update(Tracking *pTracker)
{
    ...
    point_Object_AssoID = pTracker->mCurrentFrame.keypoint_associate_objectID;
    if (pTracker->mCurrentFrame.mpReferenceKF != NULL)
    {
        {
            if (pTracker->mCurrentFrame.mnId - pTracker->mCurrentFrame.mpReferenceKF->mnFrameId < 1)
            {
                for (const QuadricLandmark *mPO : pTracker->mCurrentFrame.mpReferenceKF->mvpLocalObjects)
                {
                    cout << "AAA" << mPO->mbIsAssociated << endl;
