                                                                      ...
                                                                      mpReplaced(static_cast<MapPoint *>(NULL)), mfMinDistance(0), mfMaxDistance(0), mpMap(pMap)
{
    ...
    mnId = nNextId++;
    //added by   sjChen still less sth
    best_object = nullptr;
    max_object_vote = 0;
}
...
void MapPoint::AddObservation(KeyFrame *pKF, size_t idx)
{
    ...
    {
        int local_id = pKF->keypoint_associate_objectID[idx];
        if (local_id > -1)
        {
            AddObjectObservation(pKF->mvpLocalObjects[local_id]);
        }
    }
}
...
void MapPoint::AddObjectObservation(QuadricLandmark *obj)
{
    unique_lock<mutex> lock(mMutexObject);
    if (obj->mbIsAssociated)
    {
        QuadricLandmark *objlandmark = obj->mpAssocaitedLandmark;
...
void MapPoint::EraseObjectObservation(QuadricLandmark *obj)
{
    unique_lock<mutex> lock(mMutexObject);
...
void MapPoint::FindBestObject()
{
    ...
    best_object = nullptr;
    max_object_vote = 0;
    for (map<QuadricLandmark *, int>::iterator mit = MapObjObservations.begin(), mend = MapObjObservations.end(); mit != mend; mit++)
...
int MapPoint::GetBelongedObject(QuadricLandmark *&obj)
{
    unique_lock<mutex> lock(mMutexObject);
    obj = best_object;
...
QuadricLandmark *MapPoint::GetBelongedObject()
{
    unique_lock<mutex> lock(mMutexObject);
    return best_object;
}
