...
System::System(const string &strVocFile, const string &strSettingsFile, const int &nImages, const eSensor sensor, const int &DetectMode,
               const bool bUseViewer) : mSensor(sensor), mpViewer(static_cast<Viewer *>(NULL)), mbReset(false), mbActivateLocalizationMode(false), mbDetectMode(DetectMode),
...
void System::DetectStereo(const cv::Mat &imLeft, const cv::Mat &imRight)
{
    ...
    Frame *NewFrame = new Frame(imLeft, imRight);
    mpDetector->InsertFrame(NewFrame);
}

void System::DetectMonocular(const cv::Mat &img)
{
    if (mSensor != MONOCULAR)
    {
...
cv::Mat System::TrackStereo(const double &timestamp)
{
    if (mSensor != STEREO)
    {
        cerr << "ERROR: you called TrackStereo but input sensor was not set to STEREO." << endl;
    ...
    //cv::Mat Tcw = mpTracker->GrabImageStereo(imLeft,imRight,timestamp);
    cv::Mat Tcw = mpTracker->GrabImageStereo(timestamp); //added by  sjChen
...
cv::Mat System::TrackMonocular(const double &timestamp)
{
    if (mSensor != MONOCULAR)
    {
    ...
    cv::Mat Tcw = mpTracker->GrabImageMonocular(timestamp); //added by yxqc
