...
static int NewObjects = -1; //added by yxqc record New Objects num for generate  Quadric
                                                                                                                                                                                              ...
                                                                                                                                                                                              mpFrameDrawer(pFrameDrawer), mpMapDrawer(pMapDrawer), mpMap(pMap), mnLastRelocFrameId(0)
{
    ...
    mkdir("../kitti_raw/seq09_1frames_30/VisualImages/", 0777);
    ...
    all_kf_Twc_file.open("../kitti_raw/seq09_3frames_30/KeyFrameTwc.txt");
}
...
Frame *Tracking::GrabDetectResults()
{
    Frame *data;
    while (1)
    {
...
cv::Mat Tracking::GrabImageStereo(const double &timestamp)
{

    Frame *data = GrabDetectResults();
    mImGray = data->mLeftImg.clone();
    cv::Mat imGrayRight = data->mRightImg.clone();
    std::vector<Vector6d, Eigen::aligned_allocator<Vector6d>> boxes = data->mDetectingBoxes;
    ...
    mCurrentFrame = Frame(mImGray, imGrayRight, timestamp, boxes, mpORBextractorLeft, mpORBextractorRight, mpORBVocabulary, mK, mDistCoef, mbf, mThDepth, mpMap);
    delete data;
...
cv::Mat Tracking::GrabImageMonocular(const double &timestamp)
{
    Frame *data = GrabDetectResults();
    mImGray = data->mLeftImg;
    ...
    if (mState == NOT_INITIALIZED || mState == NO_IMAGES_YET)
        mCurrentFrame = Frame(mImGray, timestamp, boxes, mpIniORBextractor, mpORBVocabulary, mK, mDistCoef, mbf, mThDepth, mpMap);
    else
        mCurrentFrame = Frame(mImGray, timestamp, boxes, mpORBextractorLeft, mpORBVocabulary, mK, mDistCoef, mbf, mThDepth, mpMap);
    delete data;
    SaveOnlineDetectBox(&mCurrentFrame);
    PointAssociateObject(&mCurrentFrame); //added by yxqc associate keypoints with local objects
...
{
    std::string saved_dir = "../SaveDetectBox/kitti_raw/seq09/";
    ...
    ofstream file;
    file.open(save_onlinebox_txt.c_str());
    for (size_t i = 0; i < mCurrent->mvpLocalObjects.size(); i++)
    {
        QuadricLandmark *mPO = mCurrent->mvpLocalObjects[i];
        file << mPO->bbox_2d.x << " " << mPO->bbox_2d.y << " " << mPO->bbox_2d.width << " " << mPO->bbox_2d.height << " " << mPO->class_id << " " << mPO->prop << "\n";
    }
    file.close();
}
...
void Tracking::SaveTwc_Projection(KeyFrame *pKF)
{
    ...
    std::string save_projection_txt = saved_dir_projection+sequence_frame_index+".txt";
    std::string save_Twc_txt = saved_dir_Twc+sequence_frame_index+".txt";

    ofstream file_projection,file_Twc;
    file_projection.open(save_projection_txt.c_str());
    for (size_t i = 0; i < 3; i++)
        for (size_t j = 0; j < 4; j++)
        {
             file_projection <<vProject(i,j)<<" ";
        }
    file_projection.close();

    file_Twc.open(save_Twc_txt.c_str());
...
{
    pKF->keypoint_associate_objectID = vector<int>(pKF->mvKeysUn.size(), -1);
    std::vector<bool> overlapped(pKF->mvpLocalObjects.size(), false);

    if (1)
    {
        for (size_t i = 0; i < pKF->mvpLocalObjects.size(); i++)
            if (!overlapped[i])

                for (size_t j = i + 1; j < pKF->mvpLocalObjects.size(); j++)

                    if (!overlapped[j])
                    {
                        float iou_ratio = pKF->mvpLocalObjects[i]->bboxOverlapratio(pKF->mvpLocalObjects[i]->bbox_2d, pKF->mvpLocalObjects[j]->bbox_2d);
                        if (iou_ratio > 0.15)
    ...
    if (pKF->mvpLocalObjects.size() > 0)
    {
        for (size_t i = 0; i < pKF->mvKeysUn.size(); i++)
        {
            int associated_times = 0;
            for (size_t j = 0; j < pKF->mvpLocalObjects.size(); j++)
            {
                if (!overlapped[j])
                {

                    if (pKF->mvpLocalObjects[j]->bbox_2d.contains(pKF->mvKeysUn[i].pt))
                    {
                        associated_times++;
                        if (associated_times == 1)
                        {
                            pKF->keypoint_associate_objectID[i] = j;
                            pKF->mvpLocalObjects[j]->associate_points.push_back(i);
                        }
                        else
...
cv::Mat Tracking::GrabImageStereo(const cv::Mat &imRectLeft, const cv::Mat &imRectRight, const double &timestamp)
{
    ...
    mCurrentFrame = Frame(mImGray, imGrayRight, timestamp, mpORBextractorLeft, mpORBextractorRight, mpORBVocabulary, mK, mDistCoef, mbf, mThDepth, mpMap);
    PointAssociateObject(&mCurrentFrame); //added by yxqc associate keypoints with local objects
...
void Tracking::Track()
{
    ...
    else
    {
        ...
        // Update drawer   added by yxqc delay  the drawer  after create new keyframe
        /////// mpFrameDrawer->Update(this);
...
void Tracking::StereoInitialization()
{
    if (mCurrentFrame.N > 500)
    {
        ...
        if (pKFini->mvpLocalObjects.size() > 0)
        {

            for (size_t i = 0; i < pKFini->mvpLocalObjects.size(); i++)
            {
                QuadricLandmark *mLocal = pKFini->mvpLocalObjects[i];
                mLocal->SetReferenceKeyFrame(pKFini);
            }

            AssociateBbox(pKFini); //added by yxqc Bbox association or with Quadric landmark between frames
        }
...
void Tracking::CreateInitialMapMonocular()
{
    ...
    if (pKFcur->mvpLocalObjects.size() > 0)
    {

        for (size_t i = 0; i < pKFcur->mvpLocalObjects.size(); i++)
        {
            QuadricLandmark *mLocal = pKFcur->mvpLocalObjects[i];
            mLocal->SetReferenceKeyFrame(pKFcur);
        }

        AssociateBbox(pKFcur); //added by yxqc Bbox association or with Quadric landmark between frames
        InitQuadric();
    }
...
bool Tracking::NeedNewKeyFrame()
{
    ...
    if (mSensor == System::MONOCULAR)
        thRefRatio = 0.90f; ///added by yxqc raw 0.9f 

    // Condition 1a: More than "MaxFrames" have passed from last keyframe insertion
    const bool c1a = mCurrentFrame.mnId >= mnLastKeyFrameId + mMaxFrames;
    // Condition 1b: More than "MinFrames" have passed and Local Mapping is idle
    const bool c1b = (mCurrentFrame.mnId >= mnLastKeyFrameId + mMinFrames && bLocalMappingIdle);
    //Condition 1c: tracking
    const bool c1c = mSensor != System::MONOCULAR && (mnMatchesInliers < nRefMatches * 0.25 || bNeedToInsertClose);
    // added by yxqc Condition 1d: current frame has detected local objects;
    const bool c1d = mCurrentFrame.mvpLocalObjects.size() > 0;
    // Condition 2: Few tracked points compared to reference keyframe. Lots of visual odometry compared to map matches.
    const bool c2 = ((mnMatchesInliers < nRefMatches * thRefRatio || bNeedToInsertClose) && mnMatchesInliers > 15);

    if ((c1a || c1b || c1c || c1d) && c2)
    {
        ...
        else
        {
            ...
            cout << "!bLocalMappingIdle"
                 << " " << c1a << " " << c1b << " " << c1c << " " << c1d << endl;
    ...
    {
        cout << "no condition " << c1a << " " << c1b << " " << c1c << " " << c1d << " " << c2 << " " << mCurrentFrame.mnId << endl;
        return false;
    }
}
...
void Tracking::CreateNewKeyFrame()
{
    ...
    {
        if (mCurrentFrame.mnId - mCurrentFrame.mpReferenceKF->mnFrameId < 1)
            cout << "tracking" << mCurrentFrame.mnId << " " << mCurrentFrame.mpReferenceKF->mnFrameId << endl;
    }
    if (pKF->mvpLocalObjects.size() > 0)
    {
        ...
        for (size_t i = 0; i < pKF->mvpLocalObjects.size(); i++)
        {
            QuadricLandmark *mLocal = pKF->mvpLocalObjects[i];
            mLocal->SetReferenceKeyFrame(pKF);
        }

        AssociateBbox(pKF); //added by yxqc Bbox association or with Quadric landmark between frames
...
void Tracking::InitQuadric()
{
    ...
    for (size_t i = 0; i < mvpLocalKeyFrames.size(); i++)
    {
        ...
        for (size_t j = 0; j < pKF->mvpLocalObjects.size(); j++)
        {
            ...
            QuadricLandmark *mPO = pKF->mvpLocalObjects[j];
            if (mPO)
                if (!mPO->isBad() && mPO->mbIsAssociated && !mPO->mbIsInitialized)
