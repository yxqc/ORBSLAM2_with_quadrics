...
KeyFrame::KeyFrame(Frame &F, Map *pMap, KeyFrameDatabase *pKFDB) : mnFrameId(F.mnId), mTimeStamp(F.mTimeStamp), mnGridCols(FRAME_GRID_COLS), mnGridRows(FRAME_GRID_ROWS),
                                                                   ...
                                                                   mvuRight(F.mvuRight), mvDepth(F.mvDepth), mDescriptors(F.mDescriptors.clone()), /*mvpDetections(F.mvpDetections),*/
                                                                   ...
                                                                   mbToBeErased(false), mbBad(false), mHalfBaseline(F.mb / 2), mpMap(pMap)
{
    ...
    for (size_t i = 0; i < F.mvpLocalObjects.size(); i++)
    {
        mvpLocalObjects.push_back(F.mvpLocalObjects[i]);
    }
    SetPose(F.mTcw);
}
...
void KeyFrame::EraseQuadricLandmarkMatch(const size_t &idx)
{
    unique_lock<mutex> lock(mMutexFeatures);
    mvpQuadricLandmarks[idx] = static_cast<QuadricLandmark *>(NULL);
}

void KeyFrame::EraseQuadricLandmarkMatch(QuadricLandmark *pMO)
{
